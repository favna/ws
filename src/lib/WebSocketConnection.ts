...
export class WebSocketConnection {
	...
	*/
	...
	/**
	 * The options passed to the worker for this connection
	 */
	 ...
	 */
	public constructor(private parentPort: MessagePort, options: WSWorkerData) {
	...
	private sendHeartbeat(tag = 'Unknown'): void {
		if (!this.#heartbeat.acked) {
			...
			return this.destroy({ closeCode: WSCloseCodes.SessionTimeout });
		}

		this.debug(`Heartbeat[${tag}] Sending`);
		this.#heartbeat.acked = false;
		this.#heartbeat.last = Date.now();
		return this.queueWSPayload({ op: OpCodes.HEARTBEAT, d: this.#sequence }, true);
	}
	...
	private identify(): void {
		return this.#sessionID ? this.resume() : this.newSession();
	}
	...
	public newSession(): void {
		...
		if (!this.#connection) return this.newWS();

		const { options, token } = this.#options;

		this.debug(`IDENTIFY[${(options.shard as number[]).join('/')}]`);
		return this.queueWSPayload({ op: OpCodes.IDENTIFY, d: { ...(options as unknown as WSIdentify), token } }, true);
	}
	...
	private resume(): void {
		...
		return this.queueWSPayload({ op: OpCodes.RESUME, d: { seq, session_id, token: this.#options.token } }, true);
	}
